...
@Component
public class AllowedRequestQueryParamsCountFilterCustomizer implements PathItemCustomizer {
    private static final String ALLOWED_REQUEST_QUERY_PARAMS_COUNT_FILTER = "AllowedRequestQueryParamsCount";

    public AllowedRequestQueryParamsCountFilterCustomizer() {
    }
    ...
    private ApiResponses customizeResponses(RouteDefinition routeDefinition, ApiResponses responses) {
        ...
        String maxNumberOfParameters = ((String)routeDefinition.getFilters().stream().filter((f) -> {
            return f.startsWith("AllowedRequestQueryParamsCount");
        }).findFirst().get()).split("=")[1];
        ApiResponse tooManyReqResponse = (ApiResponse)responses.computeIfAbsent("414", (k) -> {
            return (new ApiResponse()).description("Request exceeded the maximum number of allowed query parameters: " + maxNumberOfParameters);
        });
    ...
    public boolean supports(RouteDefinition routeDefinition, String path) {
        return routeDefinition.getFilters().stream().anyMatch((s) -> {
            return s.startsWith("AllowedRequestQueryParamsCount=");
        });
    }
}
